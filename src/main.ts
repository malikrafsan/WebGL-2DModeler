...
let saver_tranformation_shape = undefined; // Untuk menyimpan sementara bangunan yang sedang dilakukan translasi atau dilatasi
...
const main = () => {
  ...
  function displayData() {
    ...
    shape.map((data, i) => {
      ...
      body.onclick = function(param) {
        let result = param?.target?.outerText;
  ...
  canvas.addEventListener('mousemove', function (e) {
    let transformasi = document.getElementById("transformasi")?.value;
    let x = e.offsetX / canvas.clientWidth * 2 - 1;
    let y = (1 - e.offsetY / canvas.clientHeight) * 2 - 1;
